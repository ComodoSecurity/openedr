...
//
//
macro_oss_guard::macro_oss_guard()
{
    if (internal::get_ptd()->macros_oss_busy)
        m_oss = std::make_unique<tostringstream>();
    else
    {
        internal::get_ptd()->macros_oss_busy = true;
        clear_tostringstream(internal::get_ptd()->macros_oss);
    }
}
...
{
    if (!m_oss)
        internal::get_ptd()->macros_oss_busy = false;
}

//
//
//
tostringstream& macro_oss_guard::get()
{
    return (m_oss ? *m_oss : internal::get_ptd()->macros_oss);
}
